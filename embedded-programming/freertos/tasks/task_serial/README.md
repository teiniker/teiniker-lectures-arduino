...
This thread-safety is achieved through internal **locking mechanisms** in the ESP32's hardware serial 
implementation. When one task is using the serial port, other tasks trying to access it will be 
blocked until the first task finishes its operation.

There are some considerations to keep in mind:

* **Atomic Operations**: Even though individual `Serial` calls (like 
    `Serial.print()`) are thread-safe, **a sequence of calls may not be atomic**. 

    In a multitasking environment, another task could potentially interrupt 
    between these calls, causing **mixed output** on the serial monitor. 
    If you want to ensure atomicity for a sequence of operations, then using
    a **mutex** or semaphore to protect the entire sequence would be appropriate.

* **Performance**: If you have multiple tasks frequently accessing the serial 
    port, it could lead to contention and tasks being blocked, which may impact 
    the real-time performance of your system.

* **Logical Consistency**: For logging or debugging purposes, it's often desirable 
    to have entire messages or data blocks from one task printed together, without 
    being interspersed with messages from other tasks. Mutexes can help maintain 
